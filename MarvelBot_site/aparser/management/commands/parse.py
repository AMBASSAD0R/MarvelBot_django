...
import img2pdf
import PyPDF2
#from image_to_pdf import img_to_pdf, img_to_pdf_one
from bs4 import BeautifulSoup
import json
import telebot
import sqlite3
#from sql import Sqll_comics
...
class ComicsParser:
    ...
    def get_comics_one(self, chat_id, db_comics, path1, path2):
        try:
            name = f'{path2}'
            cover = self.bot.send_photo(chat_id, open(f'{path1}\{name}\\001.jpg', 'rb'))
            msg = self.bot.send_document(chat_id, open(f'{path1}\{name}\{name}.pdf', 'rb'))
            comics_id = msg.document.file_id
            cover_id = cover.photo[0].file_id
    ...
    def run_get_comics_uni(self, path11, url, sp_num):
        for x in range(sp_num[0], sp_num[1]):
            ...
            if not os.path.exists(path1 + '/' + path1.split('/')[-1] + '.pdf'):
                ...
                img_to_pdf_one('/'.join(path1.split('/')[:-1]), path1.split('/')[-1])
                #C:/Users/zuiko/OneDrive/Desktop/MarvelBot/Comics/1602/1602 Marvel/1602 Marvel #6/1602 Marvel #6.pdf
                try:
                    #sp = list(ComicsParser.get_comics_one(self, '604900292', 'db_comics', '/'.join(path1.split('/')[:-1]), path1.split('/')[-1]))
                    try:
            ...
            #print('-------------')
            #print('/'.join(path1.split('/')[:-1]) + ' ----------------------- ', path1.split('/')[-1])
            #print('-------------')
            #print(path1.split('/')[-1])
            #try:
            #    #sp = list(ComicsParser.get_comics_one(self, '604900292', 'db_comics', '/'.join(path1.split('/')[:-1]), path1.split('/')[-1]))
        ...
        for i in json_parse['url_comics']['Marvel'].keys():
            ...
            for j in json_parse['url_comics']['Marvel'][i].keys():
                try:
                    ...
                    pass
                ComicsParser.run_get_comics_uni(self, 'C:/Users/zuiko/OneDrive/Desktop/MarvelBot/Comics/' + i + '/' + j.replace(':', " -") + '/' + j.replace(':', " -") + ' #', json_parse['url_comics']['Marvel'][i][j], [0, 200])
                #img_to_pdf_one('C:/Users/zuiko/OneDrive/Desktop/MarvelBot/Comics/' + i + '/' + j.replace(':', " -"),  j.replace(':', " -") + ' #')
                #sp = list(ComicsParser.get_comics_one(self, chat_id, 'db_comics', 'Comics/' + i + '/' + j.replace(':', " -"),  j.replace(':', " -") + ' #'))
                #p = Comics(
                #    name=sp[0],
                #    file_id = sp[1],
                #    cover_id = sp[2],
    ...
    def main(self):
        ComicsParser.parse_json(self, ComicsParser.read_json(self, self.json_path))

class Command(BaseCommand):
    help = 'Парсинг DrawnStoRies'

    def handle(self, *args, **options):
        p = ComicsParser('C:/Users/zuiko/OneDrive/Desktop/MarvelBot/data_json/comics.json', '1417817254:AAGRJdZkQSsNgWZO7Sfp8REFD1aepTPSGJg')
        p.main()
